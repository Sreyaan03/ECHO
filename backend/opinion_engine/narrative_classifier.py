...
def _build_grounded_system_prompt(registry: ReligionRegistry) -> str:
    """
...
3. **group_sensitivity**: For EACH group, how emotionally reactive will they be to this specific rumor? 
...
4. **initial_arousal_spike**: How viral is this rumor at first contact? Consider how many groups are simultaneously provoked and how high the inter-group friction is between them.
...
async def classify_narrative(text: str, registry: ReligionRegistry) -> NarrativeProfile:
    """
    Calls the LLM once at initialization to compile the rumor into a mathematical profile.
    The system prompt is grounded in the full religions.json cultural model so the LLM
    reasons over our curated data, not from scratch.
    """
    if not text or not text.strip():
        return _get_default_profile(registry)
    ...
    try:
        ...
        system_prompt = _build_grounded_system_prompt(registry)
...
def _get_default_profile(registry: ReligionRegistry) -> NarrativeProfile:
